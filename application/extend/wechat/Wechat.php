...
// :TODO: Is there any more datatype we should be in the lookout for? (Object?)
    ...
    public function getUserGroup($openid) {
        ...
        if ($result) {
            ...
            return isset($json['groupid'])?$json['groupid']:false;
        }
        return false;
    }
    ...
    public function wxOAuth2Component($appid, $scope = null) {
        ...
        if ($code) {
            ...
            $openid = $json['openid'];
            $userinfo = $this->getComponentUserInfo($openid);
            ...
            $oauth_url = $we_obj->getOauthComponentRedirect($url, $appid, "wxbase", $scope);
            redirect($oauth_url);
        }
    }
     ...
     * @return array {access_token,expires_in,refresh_token,openid,scope}
     */
    public function getOauthAccessTokenComponent($appid) {
        $code = isset($_GET['code']) ? $_GET['code'] : '';
        if (!$code)
            return false;
    ...
    public function api_create_preauthcode() {
        ...
        if ($data['expire_time'] + 600 < $now) {
            ...
            $data['component_appid'] = $this->component_appid;
            $result = $this->http_post(self::API_BASE_URL_PREFIX . self::API_CREATE_PREAUTHCODE . 'component_access_token=' . $this->component_access_token, self::json_encode($data));
            if ($result) {
                $json = json_decode($result, true);
                if (!$json || !empty($json['errcode'])) {
                    $this->errCode = $json['errcode'];
